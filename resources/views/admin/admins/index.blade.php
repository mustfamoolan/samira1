    ...
    <div x-data="adminsTable">
        ...
        <div class="pt-5">
            <div class="panel">
                ...
                <table id="adminsTable" class="whitespace-nowrap"></table>
            </div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener("alpine:init", () => {
            Alpine.data("adminsTable", () => ({
                datatable: null,
                init() {
                    this.datatable = new simpleDatatables.DataTable('#adminsTable', {
                        data: {
                            headings: ['الاسم', 'رقم الهاتف', 'تاريخ الإنشاء', 'الحالة', 'الإجراءات'],
                            data: {!! json_encode($adminsData) !!}
                        },
                        searchable: true,
                        perPage: 10,
                        ...
                        columns: [{
                                ...
                                render: (data, cell, row) => {
                                    const admin = {!! json_encode($admins) !!}[row.dataIndex];
                                    const photo = admin.photo ?
                                        `<img class="w-9 h-9 rounded-full ltr:mr-2 rtl:ml-2 object-cover" src="/storage/${admin.photo}" alt="صورة المدير" />` :
                                        `<div class="w-9 h-9 rounded-full bg-primary flex items-center justify-center text-white font-bold ltr:mr-2 rtl:ml-2">${data.charAt(0)}</div>`;
                            ...
                            },
                            {
                                ...
                                render: (data, cell, row) => {
                                    const admin = {!! json_encode($admins) !!}[row.dataIndex];
                                    const statusClass = admin.is_active ? 'success' : 'danger';
                                    const statusText = admin.is_active ? 'نشط' : 'غير نشط';
                            ...
                            },
                            {
                                ...
                                render: (data, cell, row) => {
                                    ...
                                    return `<div class="flex items-center">
                                            <a href="/admin/admins/${admin.id}/edit" class="ltr:mr-2 rtl:ml-2" x-tooltip="تعديل">
                                            ...
                                            <button type="button" onclick="deleteAdmin(${admin.id})" x-tooltip="حذف">
        ...
        function deleteAdmin(id) {
            if (confirm('هل أنت متأكد من حذف هذا المدير العام؟')) {
                const form = document.createElement('form');
                form.method = 'POST';
                form.action = `{{ route('admin.admins.destroy', '') }}/${id}`;
