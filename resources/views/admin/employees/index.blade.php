        ...
        <div class="pt-5">
            <div class="panel">
                <div class="flex items-center justify-between mb-5">
                    <h5 class="font-semibold text-lg dark:text-white-light">قائمة الموظفين</h5>
                ...
                <table id="employeesTable" class="whitespace-nowrap"></table>
            </div>
        </div>
    </div>
    ...
    <script>
                    ...
                    this.datatable = new simpleDatatables.DataTable('#employeesTable', {
                        ...
                        columns: [{
                                ...
                                render: (data, cell, row) => {
                                    const employee = {!! json_encode($employees) !!}[row.dataIndex];
                                    const photo = employee.photo ?
                                        `<img class="w-9 h-9 rounded-full ltr:mr-2 rtl:ml-2 object-cover" src="/storage/${employee.photo}" alt="صورة الموظف" />` :
                                        `<div class="w-9 h-9 rounded-full bg-warning flex items-center justify-center text-white font-bold ltr:mr-2 rtl:ml-2">${data.charAt(0)}</div>`;
                            ...
                            },
                            {
                                ...
                                render: (data, cell, row) => {
                                    ...
                                    return `<div class="flex items-center">
                                            <a href="/admin/employees/${employee.id}/edit" class="ltr:mr-2 rtl:ml-2" x-tooltip="تعديل">
                                            ...
                                            <button type="button" onclick="deleteEmployee(${employee.id})" x-tooltip="حذف">
        ...
        function deleteEmployee(id) {
            if (confirm('هل أنت متأكد من حذف هذا الموظف؟')) {
                ...
                form.method = 'POST';
                form.action = `{{ route('admin.employees.destroy', '') }}/${id}`;
